...
const LeftSidebar = ({ suggestions, events }) => {
  return (
    <div className="w-full">
      {/* Explore Section */}
      <div className="bg-white p-6 rounded-md shadow-sm mb-4">
        <h4 className="text-xl font-medium text-[#212121] mb-6">Explore</h4>
        <ul className="space-y-6">
          <li>
            <Link to="#" className="flex items-center text-[#666666] hover:text-[#1890FF] transition-colors group">
              <span className="mr-3">
              ...
              Learning
              <span className="ml-auto bg-[#0ACF83] text-white text-[10px] font-bold px-1.5 py-0.5 rounded">New</span>
            </Link>
          </li>
          <li>
            <Link to="#" className="flex items-center text-[#666666] hover:text-[#1890FF] transition-colors">
              <span className="mr-3">
          ...
          <li>
            <Link to="#" className="flex items-center text-[#666666] hover:text-[#1890FF] transition-colors">
              <span className="mr-3">
                <svg xmlns="http://www.w3.org/2000/svg" width="22" height="22" viewBox="0 0 22 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M19 21H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h11l5 5v11a2 2 0 0 1-2 2z"></path><polyline points="17 21 17 13 7 13 7 21"></polyline><polyline points="7 3 7 8 15 8"></polyline></svg>
              </span>
              Save post
            </Link>
          </li>
      ...
      <div className="bg-white p-6 rounded-md shadow-sm mb-4">
        <div className="flex justify-between items-center mb-6">
          <h4 className="text-xl font-medium text-[#212121]">Suggested People</h4>
        ...
        <div className="space-y-6">
          {suggestions && suggestions.map((person) => (
            <div key={person.id} className="flex items-center justify-between">
              ...
              <button className="text-[#666666] border border-[#E8E8E8] px-3 py-1 rounded text-sm hover:bg-gray-50">Connect</button>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="bg-white p-6 rounded-md shadow-sm">
        <div className="flex justify-between items-center mb-6">
          <h4 className="text-xl font-medium text-[#212121]">Events</h4>
          <Link to="#" className="text-[#1890FF] text-sm hover:underline">See all</Link>
        </div>
        {events && events.map((event) => (
          <div key={event.id} className="block group">
            <div className="relative rounded-md overflow-hidden mb-3">
              <img src={event.image} alt={event.title} className="w-full h-32 object-cover group-hover:scale-105 transition-transform duration-300" />
            </div>
            <div className="flex gap-3 mb-3">
              <div className="bg-[#1890FF] text-white p-2 rounded text-center min-w-[50px]">
                <span className="block text-lg font-bold leading-none">{event.date.day}</span>
                <span className="text-xs uppercase">{event.date.month}</span>
              </div>
              <div>
                <h4 className="text-[#212121] font-medium leading-tight group-hover:text-[#1890FF] transition-colors">{event.title}</h4>
              </div>
            </div>
            <div className="flex justify-between items-center border-t border-[#E8E8E8] pt-3">
              <span className="text-[#666666] text-sm">{event.going} People Going</span>
              <button className="text-[#666666] text-sm hover:text-[#1890FF]">Going</button>
            </div>
          </div>
        ))}
      </div>
